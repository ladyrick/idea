...
测试发现输出结果显示 0.1 累加 10 次并不等于 1.0。
...
逗号表达式在 C++中的运算优先级是最低的，因此，赋值运算会优先执行，导致编译器认为`2`是一个表达式，编译错误。
...
输出结果首先不是 40。事实上，如果你自己编译了的话，就可以看到显示的警告。
...
因此我们可以看到，`char**`到`const char**`的隐式类型转换是可以引发严重后果的。
...
但是，虽然躲过了宏定义的坑，其实还是错了，因为数组中的前三个数字带有前导 0，这意味着是 8 进制。。。
...
注意，现在已经不用这个了，用的是 stddef.h 自带的`offsetof`宏。它直接调用了编译器提供的函数`__builtin_offsetof`。
...
根据结合律，去掉所有括号。再根据交换律，以及`x^x`恒等于 0，可以化简为：
